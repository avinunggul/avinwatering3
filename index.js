...
if (!process.env.FIREBASE_CREDS) {
  ...
  async function getPrakiraanCuaca(kodeWilayah) {
    try {
      ...
      console.log('[Cuaca Debug] kodeWilayah:', kodeWilayah);
      console.log('[Cuaca Debug] data:', JSON.stringify(data));
  ...
  async function startBot() {
    const raspiUsers = await getAllRaspiIds();
    const raspiIdMap = Object.fromEntries(raspiUsers.map(u => [u.raspiId, u]));
    ...
    sock.ev.on('connection.update', (update) => {
      ...
      if (connection === 'open') {
        ...
        // Pasang listener Firestore untuk user baru
        firestore.collection('users').onSnapshot(snapshot => {
          snapshot.docChanges().forEach(change => {
            const data = change.doc.data();
            if (data.raspiId) {
              if (change.type === 'added') {
                // Pasang listener watering_status untuk user baru
    ...
    sock.ev.on('messages.upsert', async ({ messages }) => {
      ...
      if (text.includes('status')) {
        ...
        console.log('pesanCuaca:', pesanCuaca);
  ...
  //notifikasi 2jam prakiraan cuaca
      ...
      if (Array.isArray(item) && item.every(x => typeof x === 'number')) {
        item.forEach(idx => {
          ...
          if (
            ...
            // Ambil jam saja dari local_datetime
            let jam = '-';
  ...
  function prakiraanCuacaNotifikasi(forecasts) {
    ...
    forecasts.slice(0, 5).forEach(f => {
      let icon = '☁️';
      const cuaca = f.cuaca.toLowerCase();
      if (cuaca.includes('hujan')) icon = '🌧️';
      else if (cuaca.includes('cerah')) icon = '☀️';
      else if (cuaca.includes('berawan')) icon = '🌥️';

      pesan += `🕒 ${f.waktu} : ${f.cuaca.toUpperCase()} ${icon}\n`;
    });

    return pesan;
  }

  //notifikasi tiap 2 jam
